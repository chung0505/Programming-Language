...
reset_node_counter :-
    ...
    assert(node_count(0)),
    retractall(generated_node(_)).

increment_node_counter(Node) :-
...
bestfirst( Start, Solution) :-
    reset_node_counter,
    increment_node_counter(Start),
    expand( [], l( Start, 0/0),  9999, _, yes, Solution),
    node_count(N),
    format('~nTotal number of nodes generated: ~w~n', [N]),
    findall(X, generated_node(X), L),
    format('Total nodes generated: ~w~n', [L]).
